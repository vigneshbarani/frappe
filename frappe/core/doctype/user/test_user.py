...
class TestUser(unittest.TestCase):
	...
	def test_user_type(self):
		new_user = frappe.get_doc(dict(doctype='User', email='test-for-type@gmail.com',
	...
	def test_delete(self):
		frappe.get_doc("User", "test@gmail.com").add_roles("_Test Role 2")
		...
		if frappe.db.exists("User", "_test@gmail.com"):
			delete_contact("_test@gmail.com")
			delete_doc("User", "_test@gmail.com")

		user = frappe.copy_doc(test_records[1])
		user.email = "_test@gmail.com"
		user.insert()
	...
	def test_get_value(self):
		self.assertEqual(frappe.db.get_value("User", "test@gmail.com"), "test@gmail.com")
		self.assertEqual(frappe.db.get_value("User", {"email":"test@gmail.com"}), "test@gmail.com")
		self.assertEqual(frappe.db.get_value("User", {"email":"test@gmail.com"}, "email"), "test@gmail.com")
		self.assertEqual(frappe.db.get_value("User", {"email":"test@gmail.com"}, ["first_name", "email"]),
			("_Test", "test@gmail.com"))
		self.assertEqual(frappe.db.get_value("User",
			{"email":"test@gmail.com", "first_name": "_Test"},
			["first_name", "email"]),
				("_Test", "test@gmail.com"))

		test_user = frappe.db.sql("select * from tabUser where name='test@gmail.com'",
			as_dict=True)[0]
		self.assertEqual(frappe.db.get_value("User", {"email":"test@gmail.com"}, "*", as_dict=True),
			test_user)

		self.assertEqual(frappe.db.get_value("User", "xxxtest@gmail.com"), None)
	...
	def test_high_permlevel_validations(self):
		...
		me = frappe.get_doc("User", "testperm@gmail.com")
		me.remove_roles("System Manager")

		frappe.set_user("testperm@gmail.com")
	...
	# 		value = conn.get_value('User', 'first_name', {'name': 'test@gmail.com'})
	# 		self.assertTrue('first_name' in value)
	#
	# 	from frappe.frappeclient import FrappeClient
	# 	update_site_config('deny_multiple_sessions', 0)
	#
	# 	conn1 = FrappeClient(get_url(), "test@gmail.com", "Eastern_43A1W", verify=False)
	# 	test_request(conn1)
	#
	# 	conn2 = FrappeClient(get_url(), "test@gmail.com", "Eastern_43A1W", verify=False)
	# 	test_request(conn2)
	#
	# 	update_site_config('deny_multiple_sessions', 1)
	# 	conn3 = FrappeClient(get_url(), "test@gmail.com", "Eastern_43A1W", verify=False)
	# 	test_request(conn3)
	#
	...
	def test_delete_user(self):
		new_user = frappe.get_doc(dict(doctype='User', email='test-for-delete@gmail.com',
	...
	def test_comment_mentions(self):
		comment = '''
			<span class="mention" data-id="test.comment@gmail.com" data-value="Test" data-denotation-char="@">
				<span><span class="ql-mention-denotation-char">@</span>Test</span>
			</span>
		'''
		self.assertEqual(extract_mentions(comment)[0], "test.comment@gmail.com")
		...
		comment = '''
			<div>
				Testing comment for
				<span class="mention" data-id="test_user@gmail.com" data-value="Test" data-denotation-char="@">
					<span><span class="ql-mention-denotation-char">@</span>Test</span>
				</span>
				and
		...
		'''
		self.assertEqual(extract_mentions(comment)[0], "test_user@gmail.com")
