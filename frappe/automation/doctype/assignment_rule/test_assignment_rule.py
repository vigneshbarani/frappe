...
class TestAutoAssign(unittest.TestCase):
	...
	def test_load_balancing(self):
		...
		for user in ('test@gmail.com', 'test1@gmail.com', 'test2@gmail.com'):
		...
		for d in frappe.get_all('ToDo', dict(reference_type = 'Note', owner = 'test@gmail.com'), limit=5):
	...
	def test_clear_assignment(self):
		...
		self.assertEqual(todo.owner, 'test@gmail.com')
	...
	def check_assignment_rule_scheduling(self):
		...
		self.assertIn(frappe.db.get_value('ToDo', dict(
			...
			reference_name = note.name,
			status = 'Open'
		), 'owner'), ['test@gmail.com', 'test1@gmail.com', 'test2@gmail.com'])
...
def get_assignment_rule(days, assign=None):
	...
	assignment_rule = frappe.get_doc(dict(
		...
		users = [
			dict(user = 'test@gmail.com'),
			dict(user = 'test1@gmail.com'),
			dict(user = 'test2@gmail.com'),
		]
	)).insert()
